                    ...
                    blockStatementPath.traverse({
                        ...
                        JSXElement: path => {
                            ...
                            if (this.isInternalComponent(element)) {
                                if (this.isEmptyProps(element.attributes) && adapter_1.Adapter.type !== "swan" /* swan */) {
                                    return;
                                }
                ...
                if (body.length > 1) {
                    ...
                    if (t.isFunctionExpression(func) || t.isArrowFunctionExpression(func)) {
                        ...
                        if (utils_1.isArrayMapCallExpression(parentCallee)) {
                            ...
                            else if ("swan" /* swan */ === adapter_1.Adapter.type) {
                                const attributes = component.node.openingElement.attributes;
                                const keyAttribute = attributes.find(a => t.isJSXIdentifier(a.name, { name: 'key' }));
                                if (keyAttribute && t.isJSXExpressionContainer(keyAttribute.value)) {
                                    let itemName = itemId.name;
                                    const expressionArray = utils_1.generateMemberExpressionArray(keyAttribute.value.expression);
